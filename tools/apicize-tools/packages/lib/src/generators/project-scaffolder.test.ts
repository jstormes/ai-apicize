...
describe('ProjectScaffolder', () => {
    ...
    describe('scaffoldProject', () => {
        ...
        it('should generate configuration files', () => {
                ...
                scenarios: [
                    {
                        ...
                        name: 'Test Scenario',
                        variables: [
                            { name: 'apiKey', value: 'test123', type: VariableType.TEXT }
                        ]
                    }
                ]
            };
            ...
            expect(result.files.find(f => f.path === 'config/environments/development.json')).toBeDefined();
            expect(result.files.find(f => f.path === 'config/environments/staging.json')).toBeDefined();
            expect(result.files.find(f => f.path === 'config/environments/production.json')).toBeDefined();
