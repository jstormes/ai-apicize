...
describe('ExportPipeline', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = join(
      tmpdir(),
      `apicize-test-${Date.now()}-${Math.random().toString(36).substring(7)}`
    );
  ...
  describe('utility methods', () => {
    it('should extract project name from file path', async () => {
      ...
      const result = await exportPipeline.exportWorkbook(
        testWorkbook,
        '/path/to/my-api.apicize',
        options
      );
    ...
    it('should sanitize project names', async () => {
      ...
      const result = await exportPipeline.exportWorkbook(
        testWorkbook,
        'my api with spaces & symbols!.apicize',
        options
      );

      expect(result.success).toBe(true);
      expect(result.metadata.scaffoldedProject.metadata.projectName).toBe(
        'my-api-with-spaces---symbols-'
      );
    });
  });
