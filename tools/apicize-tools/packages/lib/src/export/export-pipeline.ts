...
import { ProjectScaffolder, ProjectScaffolderOptions, ScaffoldedProject } from '../generators/project-scaffolder';
...
export class ExportPipeline {
    ...
    public async exportWorkbook(
        ...
        const result: ExportResult = {
            ...
            metadata: {
                ...
                totalFiles: 0,
                totalDirectories: 0,
                scaffoldedProject: { files: [], metadata: { projectName: '', outputDir: '', totalFiles: 0, folders: [], scripts: [] } },
                importMappings: []
            }
        };
        ...
        try {
            ...
            const importMappings = this.generateImportMappings(workbook, sourceFilePath, scaffoldedProject);
    ...
    public async exportMultipleWorkbooks(
        ...
        for (let i = 0; i < workbooks.length; i++) {
            ...
            this.reportProgress(options, 'multi-export', fileProgress,
                `Exporting ${sourceFilePath} (${i + 1}/${workbooks.length})`);

            // Adjust output directory for multiple files
            const fileOptions: ExportOptions = {
                ...options,
                outputDir: options.outputDir ? join(options.outputDir, `workbook-${i + 1}`) : `./workbook-${i + 1}`
            };
        ...
        if (workbook.scenarios) {
            for (const scenario of workbook.scenarios) {
                mappings.push({
                    ...
                    exportedPath: join(scaffoldedProject.metadata.outputDir, 'config', 'scenarios', `${scenario.name}.json`),
                    type: 'scenario',
                    id: scenario.id,
                    metadata: {
                        name: scenario.name,
     ...
     */
    private async writeProjectFiles(scaffoldedProject: ScaffoldedProject, outputDir: string): Promise<string[]> {
